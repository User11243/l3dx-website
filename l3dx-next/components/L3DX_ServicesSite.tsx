...
const LOGO_DATA_URL = "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABgAAD/2wBDAAMCAgMCA" +
...
const PRINTER_IMG = typeof window !== "undefined" && (window as any).L3DX_PRINTER_IMG
  ...
  return (
    <section id={id} className="scroll-mt-24 py-14 md:py-20">
      <div className="mx-auto max-w-7xl px-4">
        ...
        {subtitle && <p className="mt-2 text-base md:text-lg opacity-80 max-w-3xl">{subtitle}</p>}
...
function Nav() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (visible && (visible.target as HTMLElement).id) setActive((visible.target as HTMLElement).id);
      },
  ...
  const NavLinks = ({ className = "" }: { className?: string }) => (
    <>
      {SECTIONS.map((s) => (
        <a key={s.id} href={`#${s.id}`} className={`nav-link hover:opacity-80 ${active === s.id ? "active" : ""} ${className}`}
    ...
    <header className="sticky top-0 z-50 border-b border-transparent brand-gradient text-white shadow">
      <div className="mx-auto max-w-7xl px-4 h-16 flex items-center justify-between">
        <a href="#hero" className="flex items-center gap-2 font-bold text-xl">
          <img src={LOGO_DATA_URL} alt="L3DX" className="h-8 w-8 rounded-full ring-1 ring-white/30 object-cover" />
          <span>L3DX</span>
        </a>
        <nav className="hidden md:flex items-center gap-6 text-sm">
          <NavLinks />
          <div className="h-5 w-px bg-white/20" />
          <button onClick={() => setDark(!dark)} className="inline-flex items-center gap-2 text-sm px-3 py-1.5 rounded-xl border border-white/30 hover:bg-white/10">
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden inline-flex items-center gap-2 text-sm px-3 py-1.5 rounded-xl border border-white/30">
          Меню
        </button>
      </div>
      {open && (
        <div className="md:hidden border-t border-white/20 bg-black/10">
          <div className="px-4 py-3 flex flex-col gap-2">
            ...
            <div className="pt-2 flex items-center gap-3">
              <button onClick={() => setDark(!dark)} className="inline-flex items-center gap-2 text-sm px-3 py-1.5 rounded-xl border border-white/30">
                {dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
                Тема
              </button>
              <a href="#order" className="inline-flex items-center gap-2 text-sm btn-primary">
      ...
      <div className="mx-auto max-w-7xl px-4 pt-16 md:pt-24 pb-10 md:pb-16">
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-semibold leading-tight tracking-tight">3D принтиране по поръчка от <span className="underline decoration-2 underline-offset-4">L3DX</span></h1>
            <p className="mt-4 text-lg opacity-90 max-w-2xl">Превръщаме вашите идеи и модели в реални продукти. Работим с PLA, PETG, ABS, ASA, TPU и др. Приемаме и само линк към модел от платформи като Printables, Thingiverse, MakerWorld.</p>
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-xs opacity-70">*Реалният принтер, с който работим. При смяна на файла "/P1.jpg" или задаване на window.L3DX_PRINTER_IMG, снимката се обновява.</p>
          </div>
        </div>
      </div>
    </section>
  ...
  const items = [
    {
      ...
      desc: "Изпратете STL/3MF и ще предложим най-подходящия материал, слой и запълване според приложението.",
    },
    {
      icon: LinkIcon,
      title: "Печат на файлове от интернет",
      desc: "Нямате модел? Изпратете линк от Printables / Thingiverse / MakerWorld и ние ще се погрижим за останалото.",
    },
    {
      icon: Boxes,
      title: "Единични и малки серии",
      desc: "От единични бройки до кратки серии за прототипи, резервни части, RC/авто аксесоари, подаръци и др.",
    },
    {
      icon: Wrench,
      title: "Консултация и съдействие",
      desc: "Помагаме с избор на материал, мащаб, толеранси, резби, вложки, ориентация и следобработка.",
    },
  ];

  return (
    <Section id="services" title="Услуги" subtitle="Гъвкави, бързи и на разумни цени — по подобие на популярните услуги на пазара, с акцент върху персонализацията.">
  ...
  return (
    <Section id="materials" title="Материали" subtitle="Подбираме материала според целта — декоративно, функционално, за вън или с гъвкавост.">
      ...
      <p className="mt-4 text-sm opacity-70">
        * Предлагаме и по-специални пластмаси при запитване (напр. усилени с влакна). Толеранси по подразбиране: ±0.2 мм за стандартни детайли.
      </p>
    </Section>
  );
}
...
function Pricing() {
  ...
  return (
    <Section id="pricing" title="Цени" subtitle="Двувалутно показване (BGN/€) с фиксиран курс. Калкулаторът е ориентировъчен — точна оферта изпращаме след преглед на модела.">
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Таблица със ставки */}
        <div className="lg:col-span-1 rounded-2xl border brand-card p-5">
          <h3 className="font-semibold text-lg flex items-center gap-2"><Euro className="h-5 w-5" /> Базови ставки</h3>
          <div className="mt-4 space-y-3 text-sm">
            ...
            <p className="mt-3 text-xs opacity-70">Пълната цена зависи от сложност, следобработка, материални премии и срок.</p>
          </div>
        </div>

        {/* Калкулатор */}
        <div className="lg:col-span-2 rounded-2xl border brand-card p-5">
          <h3 className="font-semibold text-lg flex items-center gap-2"><Calculator className="h-5 w-5" /> Калкулатор</h3>
          <div className="mt-4 grid md:grid-cols-3 gap-4 text-sm">
            <label className="grid gap-1">
              Материал
              <select value={mat} onChange={(e) => setMat(e.target.value as any)} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2">
            ...
            <label className="grid gap-1">
              Грамаж (г)
              <input type="number" min={0} value={grams} onChange={(e) => setGrams(Math.max(0, Number(e.target.value)))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
            </label>
            <label className="grid gap-1">
              Машинно време (ч)
              <input type="number" min={0} value={machineH} onChange={(e) => setMachineH(Math.max(0, Number(e.target.value)))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
            </label>
            <label className="grid gap-1">
              Ръчна работа (ч)
              <input type="number" min={0} value={laborH} onChange={(e) => setLaborH(Math.max(0, Number(e.target.value)))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
            </label>
          ...
          <p className="mt-3 text-xs opacity-70">Пример: 150 г, 5 ч, без ръчна работа, PLA → виж изчислението по-горе. Минималната поръчка може да се приложи.
          </p>
            ...
            <div className="mt-3 grid md:grid-cols-2 gap-3 text-sm">
              {Object.keys(pricesBG.materials).map((name) => (
                <label key={name} className="grid gap-1">
                  {name} (лв/г)
                  <input type="number" min={0} step={0.01} value={(pricesBG.materials as any)[name]} onChange={(e) => setPricesBG((p) => ({ ...p, materials: { ...p.materials, [name]: Number(e.target.value) } }))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
                </label>
              ))}
              <label className="grid gap-1">
                Машинно време (лв/ч)
                <input type="number" min={0} step={0.1} value={pricesBG.machineHour} onChange={(e) => setPricesBG((p) => ({ ...p, machineHour: Number(e.target.value) }))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
              </label>
              <label className="grid gap-1">
                Ръчна работа (лв/ч)
                <input type="number" min={0} step={0.1} value={pricesBG.laborHour} onChange={(e) => setPricesBG((p) => ({ ...p, laborHour: Number(e.target.value) }))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
              </label>
              <label className="grid gap-1">
                Минимална поръчка (лв)
                <input type="number" min={0} step={0.1} value={pricesBG.minOrder} onChange={(e) => setPricesBG((p) => ({ ...p, minOrder: Number(e.target.value) }))} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" />
              </label>
            </div>
          </details>
        </div>
    ...
    <Section id="order" title="Поръчка" subtitle="Изпратете линк към модела или кратко описание. Ще се върнем с оферта и срок.">
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border brand-card p-5">
          <h3 className="font-semibold text-lg flex items-center gap-2"><Upload className="h-5 w-5" /> Форма за запитване</h3>
          <form className="mt-4 grid gap-3 text-sm" onSubmit={(e) => e.preventDefault()}>
            <label className="grid gap-1">Име<input className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" placeholder="Иван Иванов"/></label>
            <label className="grid gap-1">Имейл<input className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" placeholder="you@example.com"/></label>
            <label className="grid gap-1">Линк към модел<input className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" placeholder="https://www.printables.com/..."/></label>
            <label className="grid gap-1">Описание<textarea rows={4} className="rounded-xl border border-neutral-900/10 dark:border-white/10 bg-transparent px-3 py-2" placeholder="Материал, цвят, размер, изисквания, срок..."/></label>
            <div className="flex items-center justify-between">
              <p className="text-xs opacity-70">* Може да изпратите и директно имейл с файлове/линк.</p>
              <a href="mailto:l3dx@abv.bg?subject=Запитване%20за%203D%20печат%20(L3DX)" className="inline-flex items-center gap-2 btn-primary"><Mail className="h-4 w-4"/> Изпрати имейл</a>
            </div>
          </form>
        </div>
        <div className="rounded-2xl border brand-card p-5">
          <h3 className="font-semibold text-lg flex items-center gap-2"><Shield className="h-5 w-5" /> Политики</h3>
          ...
          <p className="mt-3 text-xs opacity-70">Пълни Общи условия и Политика за лични данни ще бъдат публикувани на l3dx.bg.</p>
        </div>
      </div>
    </Section>
  );
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6">
        {faqs.map((f) => (
          <details key={f.q} className="rounded-2xl border brand-card p-5">
            <summary className="cursor-pointer font-medium flex items-center gap-2"><HelpCircle className="h-5 w-5" /> {f.q}</summary>
...
function Diagnostics() {
  ...
  const tests: Test[] = [
    ...
    { name: "currency: 19.5583 BGN ≈ €10", pass: Math.abs(19.5583 / FIXED_EUR_RATE - 10/ (1.0)) < 0.02, details: "≈ €10.00" },
    { name: "assets: PRINTER_IMG string", pass: typeof PRINTER_IMG === "string" && PRINTER_IMG.length > 0 },
  ...
  return (
    <Section id="diagnostics" title="Тестове (dev)" subtitle="Мини проверка на икони и формули за ценообразуване.">
      <div className="rounded-2xl border brand-card p-5 text-sm">
  ...
  return (
    <Section id="contacts" title="Контакти" subtitle="Работим онлайн за цяла България. За въпроси и оферти — пишете ни.">
      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-2xl border brand-card p-5">
          <h3 className="font-semibold text-lg flex items-center gap-2"><Mail className="h-5 w-5" /> Имейл</h3>
          <a href="mailto:l3dx@abv.bg" className="mt-2 inline-block underline underline-offset-4 brand-link">l3dx@abv.bg</a>
          <p className="mt-3 text-sm opacity-80">Може да изпратите и директно файлове (STL/3MF) или линк към модела.</p>
        </div>
        <div className="rounded-2xl border brand-card p-5">
          <h3 className="font-semibold text-lg flex items-center gap-2"><Layers className="h-5 w-5" /> L3DX</h3>
          <p className="mt-2 text-sm opacity-80">l3dx.bg — услуги за 3D принтиране и дизайн. База: Варна (работим изцяло онлайн).</p>
          <p className="mt-2 text-xs opacity-60">Авторски права върху моделите са собственост на техните притежатели. Приемаме поръчки при спазване на IP правилата.</p>
        </div>
      </div>
    </Section>
  );
